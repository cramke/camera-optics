...
pub fn calculate_dori_parameter_ranges(
    ...
    if let Some(focal) = constraints.focal_length_mm {
        if let Some(sensor_w) = constraints.sensor_width_mm {
            ...
            ranges.horizontal_fov_deg = Some(ParameterRange { min: fov, max: fov });
        ...
        let min_sensor =
            (MIN_FOCAL_LENGTH_MM * pixels as f64) / (target_distance * required_px_per_m);
        // For maximum sensor width, use maximum focal length
        let max_sensor =
            (MAX_FOCAL_LENGTH_MM * pixels as f64) / (target_distance * required_px_per_m);
    ...
    if constraints.horizontal_fov_deg.is_none() {
        ...
        if let (Some(sensor_range), Some(focal_range)) =
            (&ranges.sensor_width_mm, &ranges.focal_length_mm)
        {
...
mod tests {
    ...
    fn test_dori_ranges_with_fov_constraint() {
        ...
        if let Some(pixel_range) = &ranges.pixel_width {
            ...
            assert!(
                pixel_range.min > 2800.0,
                "Min pixels should be constrained by FOV: {}",
                pixel_range.min
            );
            assert!(
                pixel_range.min < 3000.0,
                "Min pixels should be around 2887: {}",
                pixel_range.min
            );
        }

        // Verify that sensor/focal maintain the FOV relationship
        if let (Some(focal_range), Some(sensor_range)) =
            (&ranges.focal_length_mm, &ranges.sensor_width_mm)
        {
            // Check that the FOV is maintained at both extremes
            let fov_at_min = 2.0
                * (sensor_range.min / (2.0 * focal_range.min))
                    .atan()
                    .to_degrees();
            let fov_at_max = 2.0
                * (sensor_range.max / (2.0 * focal_range.max))
                    .atan()
                    .to_degrees();

            // Both should be close to 60 degrees
            assert!(
                (fov_at_min - 60.0).abs() < 1.0,
                "FOV at min is {}, expected 60",
                fov_at_min
            );
            assert!(
                (fov_at_max - 60.0).abs() < 1.0,
                "FOV at max is {}, expected 60",
                fov_at_max
            );
        }
    }
    ...
    fn test_dori_ranges_calculates_height_dimensions() {
        ...
        if let (Some(sensor_w), Some(sensor_h)) =
            (&ranges.sensor_width_mm, &ranges.sensor_height_mm)
        {
            ...
            // Should be 4:3 = 1.333...
            assert!(
                (aspect_ratio_min - 1.333).abs() < 0.01,
                "Min aspect ratio should be 4:3"
            );
            assert!(
                (aspect_ratio_max - 1.333).abs() < 0.01,
                "Max aspect ratio should be 4:3"
            );
        }
        ...
        if let (Some(pixel_w), Some(pixel_h)) = (&ranges.pixel_width, &ranges.pixel_height) {
            ...
            assert!(
                (aspect_ratio_min - 1.333).abs() < 0.01,
                "Min pixel aspect ratio should be 4:3"
            );
            assert!(
                (aspect_ratio_max - 1.333).abs() < 0.01,
                "Max pixel aspect ratio should be 4:3"
            );
        }
    }
    ...
    fn test_dori_ranges_fov_with_sensor_determines_focal() {
        ...
        assert!(
            ranges.focal_length_mm.is_some(),
        ...
        if let Some(focal) = &ranges.focal_length_mm {
            ...
            println!(
                "Expected focal: {}, Got: min={}, max={}",
                expected_focal, focal.min, focal.max
            );

            assert!(
                (focal.min - expected_focal).abs() < 0.1,
                "Min focal should be ~30mm"
            );
            assert!(
                (focal.max - expected_focal).abs() < 0.1,
                "Max focal should be ~30mm"
            );
            assert!(
                (focal.min - focal.max).abs() < 0.01,
                "Min and max should be the same (determined value)"
            );
        }

        // Height dimensions should also be calculated with 4:3 aspect ratio
        assert!(
            ranges.sensor_height_mm.is_some(),
            "Sensor height should be calculated"
        );
        assert!(
            ranges.pixel_height.is_some(),
        ...
        if let Some(sensor_h) = &ranges.sensor_height_mm {
            ...
            println!(
                "Expected sensor height: {}, Got: min={}, max={}",
                expected_height, sensor_h.min, sensor_h.max
            );
            assert!(
                (sensor_h.min - expected_height).abs() < 0.01,
                "Sensor height should be 3.15mm"
            );
            assert!(
                (sensor_h.max - expected_height).abs() < 0.01,
                "Sensor height should be 3.15mm"
            );
        }
        ...
        if let Some(pixel_h) = &ranges.pixel_height {
            ...
            println!(
                "Expected pixel height: {}, Got: min={}, max={}",
                expected_height, pixel_h.min, pixel_h.max
            );
            assert!(
                (pixel_h.min - expected_height).abs() < 0.01,
                "Pixel height should be 4500"
            );
            assert!(
                (pixel_h.max - expected_height).abs() < 0.01,
                "Pixel height should be 4500"
            );
        }
    }
    ...
    fn test_dori_ranges_fov_with_focal_determines_sensor() {
        ...
        assert!(
            ranges.sensor_width_mm.is_some(),
            "Sensor width should be calculated"
        );
        ...
        if let Some(sensor) = &ranges.sensor_width_mm {
            ...
            println!(
                "Expected sensor: {}, Got: min={}, max={}",
                expected_sensor, sensor.min, sensor.max
            );

            assert!(
                (sensor.min - expected_sensor).abs() < 0.1,
                "Min sensor should be ~28.87mm"
            );
            assert!(
                (sensor.max - expected_sensor).abs() < 0.1,
                "Max sensor should be ~28.87mm"
            );
            assert!(
                (sensor.min - sensor.max).abs() < 0.01,
                "Min and max should be the same (determined value)"
            );
        }
    }
    ...
    fn test_dori_ranges_sensor_with_focal_determines_fov() {
        ...
        assert!(
            ranges.horizontal_fov_deg.is_some(),
        ...
        if let Some(fov) = &ranges.horizontal_fov_deg {
            ...
            println!(
                "Expected FOV: {}, Got: min={}, max={}",
                expected_fov, fov.min, fov.max
            );

            assert!(
                (fov.min - expected_fov).abs() < 0.01,
                "Min FOV should be ~3.82°"
            );
            assert!(
                (fov.max - expected_fov).abs() < 0.01,
                "Max FOV should be ~3.82°"
            );
            assert!(
                (fov.min - fov.max).abs() < 0.01,
                "Min and max should be the same (determined value)"
            );
        }
    }
    ...
    fn test_combo_none_none_none_none() {
        ...
        assert!(
            ranges.sensor_width_mm.is_some(),
            "Sensor width should have range"
        );
        assert!(
            ranges.pixel_width.is_some(),
            "Pixel width should have range"
        );
        assert!(
            ranges.focal_length_mm.is_some(),
            "Focal length should have range"
        );
        assert!(ranges.horizontal_fov_deg.is_some(), "FOV should have range");
        assert!(
            ranges.sensor_height_mm.is_some(),
            "Sensor height should have range"
        );
        assert!(
            ranges.pixel_height.is_some(),
            "Pixel height should have range"
        );
    ...
    fn test_combo_sensor_none_none_none() {
        ...
        assert!(
            ranges.sensor_width_mm.is_none(),
            "Sensor width should be None (fixed input)"
        );
    ...
    fn test_combo_none_pixel_none_none() {
        ...
        assert!(
            ranges.pixel_width.is_none(),
            "Pixel width should be None (fixed input)"
        );
    ...
    fn test_combo_sensor_pixel_none_none() {
        ...
        if let Some(focal) = &ranges.focal_length_mm {
            ...
            assert!(
                (focal.min - expected_min).abs() < 0.5,
                "Min focal should be ~{}, got {}",
                expected_min,
                focal.min
            );
        }
    ...
    fn test_combo_none_none_focal_none() {
        ...
        assert!(
            ranges.focal_length_mm.is_none(),
            "Focal length should be None (fixed input)"
        );
        // Others should have ranges
        assert!(
            ranges.sensor_width_mm.is_some(),
    ...
    fn test_combo_sensor_none_focal_none() {
        ...
        if let Some(fov) = &ranges.horizontal_fov_deg {
            ...
            assert!(
                (fov.min - expected).abs() < 0.1,
                "FOV should be ~{}",
                expected
            );
            assert!(
                (fov.min - fov.max).abs() < 0.01,
                "FOV should be single value"
            );
        }
    }
    ...
    fn test_combo_sensor_pixel_focal_none() {
        ...
        assert!(
            ranges.pixel_width.is_some(),
            "Pixel width range should show DORI requirement"
        );
        assert!(
            ranges.sensor_width_mm.is_none(),
            "Sensor width should be None (fixed)"
        );
        assert!(
            ranges.focal_length_mm.is_none(),
            "Focal length should be None (fixed)"
        );
        // FOV should be calculated
        assert!(
            ranges.horizontal_fov_deg.is_some(),
    ...
    fn test_combo_none_none_none_fov() {
        ...
        assert!(
            ranges.horizontal_fov_deg.is_none(),
            "FOV should be None (fixed input)"
        );
    ...
    fn test_combo_sensor_none_none_fov() {
        ...
        if let Some(focal) = &ranges.focal_length_mm {
            ...
            assert!(
                (focal.min - expected).abs() < 0.5,
                "Focal should be ~{}",
                expected
            );
            assert!(
                (focal.min - focal.max).abs() < 0.01,
                "Focal should be single value"
            );
        }
    }
    ...
    fn test_combo_none_none_focal_fov() {
        ...
        if let Some(sensor) = &ranges.sensor_width_mm {
            ...
            assert!(
                (sensor.min - expected).abs() < 0.5,
                "Sensor should be ~{}",
                expected
            );
            assert!(
                (sensor.min - sensor.max).abs() < 0.01,
                "Sensor should be single value"
            );
        }
    }
    ...
    fn test_combo_sensor_none_focal_fov() {
        ...
        assert!(
            ranges.sensor_width_mm.is_some(),
            "Sensor width should be calculated for validation"
        );
        assert!(
            ranges.focal_length_mm.is_none(),
            "Focal length should be None (fixed input)"
        ...
        if let Some(sensor) = &ranges.sensor_width_mm {
            ...
            assert!(
                (sensor.min - expected).abs() < 1.0,
                "Calculated sensor should match FOV+focal relationship: {} vs {}",
                sensor.min,
                expected
            );
            assert!(
                (sensor.min - 36.0).abs() < 1.0,
    ...
    fn test_combo_sensor_pixel_focal_fov() {
        ...
        assert!(
            ranges.sensor_width_mm.is_some(),
            "Sensor width calculated for validation"
        );
        assert!(
            ranges.pixel_width.is_none(),
            "Pixel width should be None (fixed input)"
        ...
        if let Some(sensor) = &ranges.sensor_width_mm {
            assert!(
                (sensor.min - 6.4).abs() < 1.0,
                "Calculated sensor should approximately match input: {}",
                sensor.min
            );
        }
    }
    ...
    fn test_height_sensor_height_only() {
        ...
        assert!(
            ranges.sensor_height_mm.is_none(),
            "Sensor height should be None (fixed input)"
        );
    ...
    fn test_height_pixel_height_only() {
        ...
        assert!(
            ranges.pixel_height.is_none(),
            "Pixel height should be None (fixed input)"
        );
        // Other parameters should have ranges
        assert!(
            ranges.sensor_width_mm.is_some(),
    ...
    fn test_height_sensor_width_and_height() {
        ...
        assert!(
            ranges.sensor_height_mm.is_none(),
            "Sensor height should be None (fixed)"
        );
        // Other parameters should have ranges
        assert!(
            ranges.pixel_width.is_some(),
    ...
    fn test_height_pixel_width_and_height() {
        ...
        assert!(
            ranges.pixel_width.is_none(),
            "Pixel width should be None (fixed)"
        );
        assert!(
            ranges.pixel_height.is_none(),
            "Pixel height should be None (fixed)"
        );
        // Other parameters should have ranges
        assert!(
            ranges.sensor_width_mm.is_some(),
    ...
    fn test_height_sensor_and_focal_with_sensor_height() {
        ...
        if let Some(pixel_h) = &ranges.pixel_height {
            if let Some(pixel_w) = &ranges.pixel_width {
                ...
                assert!(
                    (aspect_min - 4.0 / 3.0).abs() < 0.01,
                    "Min aspect should be 4:3"
                );
                assert!(
                    (aspect_max - 4.0 / 3.0).abs() < 0.01,
                    "Max aspect should be 4:3"
                );
            }
        }
    }
    ...
    fn test_height_vertical_fov_implications() {
        ...
        assert!(
            ranges.horizontal_fov_deg.is_some(),
            "Horizontal FOV should be calculated"
        );
        ...
        if let Some(h_fov) = &ranges.horizontal_fov_deg {
            ...
            assert!(
                (h_fov.min - expected_h).abs() < 0.5,
                "Horizontal FOV should be ~{}",
                expected_h
            );
        }
    }
